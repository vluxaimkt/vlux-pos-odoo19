...
SOURCE_STOCK_QTY="$(pg_query "vlux-${TENANT}-postgres" "$DB" "SELECT to_char(coalesce(sum(q.quantity), 0), 'FM999999999.####') FROM stock_quant q JOIN stock_location l ON l.id = q.location_id WHERE q.product_id = ${SOURCE_STOCK_PRODUCT_ID} AND l.usage = 'internal'")"
...
DST_STOCK_QTY="$(pg_query "$B_PG" "$DB" "SELECT to_char(coalesce(sum(q.quantity), 0), 'FM999999999.####') FROM stock_quant q JOIN stock_location l ON l.id = q.location_id WHERE q.product_id = ${DST_STOCK_PRODUCT_ID} AND l.usage = 'internal'")"
